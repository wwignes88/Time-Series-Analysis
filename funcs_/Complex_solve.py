...
# set to zero. Edit values as needed later.
    ...
    if np.all(abs(x)==0) == True and dep == False:
        return 'independent', np.matrix(x), rank_ , x_solve.astype('int'), M, E

    if np.any(np.isnan(x)==True):
        return 'NA', np.matrix(x), rank_ , x_solve.astype('int'), M, E

    else:
        return 'dependent', np.matrix(x), rank_ ,  x_solve.astype('int'), M, E
